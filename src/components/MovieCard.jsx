  ...
  return (
    <div className="group">
      ...
      <div className="relative  rounded-lg border border-slate-200 bg-white">
        ...
        <img src={movie.Image} alt={movie.Name} className="h-95 w-full object-cover" />

        {/* Hover Overlay */}
        <div className="absolute inset-0 flex items-center justify-center bg-black/60 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          <div className="flex w-35 flex-col gap-3">
            <button className="rounded border border-white py-2 text-sm text-white transition hover:bg-white hover:text-black">Details</button>

            <button className="rounded bg-primary py-2 text-sm text-white transition hover:bg-primary/80">Buy Ticket</button>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {movie.Categories?.map((item) => (
            <span key={item} className="rounded-full bg-slate-100 px-4 py-1 text-sm text-slate-500">
              {item.trim()}
            </span>
          ))}
        </div>
